...
############ 1. run prediction model using 'ctree' package ###############
...
############ 2. LOGIT model and generate output ###############
...
############ 3. run prediction model using 'randomForest' package ###############
...
############ 4. run prediction model using 'rpart' package ###############
...
ggplot(data = varImpdf,
  ...
  ylab("Variable Importance") + 
  geom_bar(stat="identity") +
  ggtitle("Kaggle Titanic: Variable Importance Ratings using 'RPART'")


#specify levels for 'title' as differs between test and train
levels(titanic_test$title) <- unique(titanic_combi$title)

# model execution
Prediction <- predict(fit, titanic_test)                 

# create dataframe and submission file for Kaggle
submit <- data.frame(PassengerId = titanic_test$PassengerId, Survived = as.vector(round(Prediction[,2], digits=0)))
